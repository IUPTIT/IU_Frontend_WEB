      ...
      {phase === "closed" ? (
        ...
        <div className="mt-6 flex flex-wrap items-center justify-center gap-2 sm:gap-3">
          {[
            ...
            <div key={unit} className="liquid-glass landing-card-glass rounded-2xl px-3 py-2 text-center sm:px-4 sm:py-3">
              <p className="landing-headline text-xl font-semibold text-[hsl(var(--landing-foreground))] sm:text-2xl">
                {String(value).padStart(2, "0")}
              </p>
              <p className="text-xs text-[hsl(var(--landing-foreground)/0.5)]">{unit}</p>
            </div>
          ))}
          <p className="w-full text-center text-sm text-[hsl(var(--landing-foreground)/0.6)] sm:ml-2 sm:w-auto">
