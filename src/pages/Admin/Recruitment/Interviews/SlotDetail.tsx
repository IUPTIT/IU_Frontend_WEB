      ...
      <section className="neu-card !p-0 overflow-hidden">
        <div className="flex items-center justify-between border-b border-black/5 px-5 py-4">
          ...
          <p className="text-sm text-muted">
            <span className="font-semibold text-foreground">{candidates.length}</span>/{slot.capacity} chỗ
          </p>
        </div>
          ...
          <table className="w-full min-w-[760px] text-left">
            <thead>
              <tr className="bg-accent/15 text-xs uppercase tracking-wide text-accent">
                ...
                <th className="px-3 py-3.5 font-semibold text-center">Điểm PV</th>
                <th className="px-3 py-3.5 font-semibold text-center">Người chấm</th>
              ...
              {candidates.length === 0 ? (
                ...
                candidates.map((c) => (
                  <tr
                    ...
                    onClick={() => navigate(ROUTES.admin.recruitment.interviewNote(c.bookingId))}
                  >
                    <td className="px-4 py-4 text-sm font-semibold text-accent">{c.applicationCode}</td>
